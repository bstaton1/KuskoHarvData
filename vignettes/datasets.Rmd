...
subtitle: '`r htmltools::tagList(paste0("Version: ", packageVersion("KuskoHarvData")), htmltools::img(src="../man/figures/sticker/KuskoHarvData-logo.png", align = "right", height = "200px", style = "margin-top: -75px;"))`'
output: 
...
```{r set-css}
# select the CSS file
css_file = system.file("templates/KuskoHarv-vignette-styles.css", package = "KuskoHarvUtils")
...
knitr::include_graphics("C:/Users/bstaton/Desktop/Staton/5_qes/01-projects/2021_KuskoHarvPred_AYKSSI/03-manuscripts/KuskoHarvEst-ms/2-fig/map.png")
```
...
```{r harvest_estimate_master-load, echo = TRUE}
...
```{r harvest_estimate_master-vars}
matrix(c(
  # VARIABLE                              # DESCRIPTION
...
```{r harvest_estimate_master-head}
data_table(harvest_estimate_master, head)
```
...
```{r harvest_estimate_master-tail}
data_table(harvest_estimate_master, tail)
```
...
```{r harvest_estimate_master-summary}
summary(harvest_estimate_master)
```
...
```{r interview_data_master-load, echo = TRUE}
...
```{r interview_data_master-head}
data_table(interview_data_master, head)
```
...
```{r interview_data_master-tail}
data_table(interview_data_master, tail)
```
...
```{r interview_data_master-summary}
summary(interview_data_master)
```
...
```{r flight_data_master-load, echo = TRUE}
...
```{r flight_data_master-head}
data_table(flight_data_master, head)
```
...
```{r flight_data_master-tail}
data_table(flight_data_master, tail)
```
...
```{r flight_data_master-summary}
summary(flight_data_master)
```
