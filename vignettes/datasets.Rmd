...
**NOTE:** BTF operations concluded in 2024 and new data are `r htmltools::tags$u("unlikely")` to be available in future years. 
:::
...
## `r fa("table")` Kuskokwim Sonar Count Records

::::::{.indent}

```{r sonar_data_all-load, echo = TRUE}
data(sonar_data_all, package = "KuskoHarvData")
```

:::{.describe}
Each row represents the daily count for one species, each day of sonar operations (`r paste(range(lubridate::year(sonar_data_all$date)), collapse = "-")`).
Data are available from the [ADF&G Kuskokwim Escapement Fish Counts website](https://www.adfg.alaska.gov/index.cfm?adfg=commercialbyareakuskokwim.emihd). -- see step 5 in `vignette("updating-data", package = "KuskoHarvData")` for details on updating this data set.

**NOTE:** Sonar operations began in 2018 and new data are `r htmltools::tags$u("likely")` to be available in future years. 
:::

<details>
<summary>Variable Descriptions</summary>

```{r sonar_data_all-vars}
matrix(c(
  # VARIABLE    # DESCRIPTION
  "date",       "Date of sampling",
  "species",    "Species for which the count applies",
  "count",      "Count of fish -- the estimated daily passage of that species past the sonar site",
  "ccount",      "Cumulative count; the sum of all daily counts up to and including the value in the `date` column"
), ncol = 2, byrow = TRUE) |> 
vars_table()
```
...
```{r sonar_data_all-head}
data_table(sonar_data_all, head)
```
...
```{r sonar_data_all-tail}
data_table(sonar_data_all, tail)
```
...
```{r sonar_data_all-summary}
summary(sonar_data_all)
```
